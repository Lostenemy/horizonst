...
type LandingArtwork = { src: string; mobileSrc: string; width: number; height: number; alt: string; sizes: string };

export const landingArtwork = {
  hero: { src: '/images/campaign-comic/05-entering-freezer.webp', mobileSrc: '/images/campaign-comic/05-entering-freezer-mobile.webp', width: 1400, height: 788, alt: 'Trabajador equipado entrando en una gran cámara congeladora', sizes: '(max-width: 860px) 100vw, 48vw' },
  intro: { src: '/images/campaign-comic/06-buddy-check.webp', mobileSrc: '/images/campaign-comic/06-buddy-check-mobile.webp', width: 1400, height: 933, alt: 'Dos integrantes del equipo comprueban sus dispositivos antes de entrar', sizes: '(max-width: 860px) 100vw, 46vw' },
  alert: { src: '/images/campaign-comic/19-mobile-alert.webp', mobileSrc: '/images/campaign-comic/19-mobile-alert-mobile.webp', width: 1122, height: 1402, alt: 'Responsable recibiendo una alerta en el móvil junto a una cámara frigorífica', sizes: '(max-width: 860px) 58vw, 28vw' },
  response: { src: '/images/campaign-comic/08-alert-response.webp', mobileSrc: '/images/campaign-comic/08-alert-response-mobile.webp', width: 1400, height: 933, alt: 'Equipo accediendo a una cámara frigorífica para comprobar una alerta', sizes: '(max-width: 860px) 42vw, 24vw' },
  review: { src: '/images/campaign-comic/15-incident-review.webp', mobileSrc: '/images/campaign-comic/15-incident-review-mobile.webp', width: 1400, height: 758, alt: 'Responsables revisando registros operativos en un portátil', sizes: '(max-width: 860px) 42vw, 24vw' },
  sectors: [
    { src: '/images/campaign-comic/09-frozen-bakery.webp', mobileSrc: '/images/campaign-comic/09-frozen-bakery-mobile.webp', width: 1122, height: 1402, alt: 'Trabajador supervisando producto en una cámara congeladora de panadería', sizes: '(max-width: 640px) 100vw, 44vw' },
    { src: '/images/campaign-comic/11-pharma-cold-room.webp', mobileSrc: '/images/campaign-comic/11-pharma-cold-room-mobile.webp', width: 1400, height: 933, alt: 'Trabajador comprobando su dispositivo en una cámara de almacenamiento farmacéutico', sizes: '(max-width: 640px) 100vw, 44vw' }
  ],
  guide: { src: '/images/campaign-comic/14-team-onboarding.webp', mobileSrc: '/images/campaign-comic/14-team-onboarding-mobile.webp', width: 1400, height: 788, alt: 'Equipo recibiendo formación práctica antes de trabajar en frío', sizes: '(max-width: 860px) 100vw, 46vw' },
  plans: { src: '/images/campaign-comic/18-charging-fleet.webp', mobileSrc: '/images/campaign-comic/18-charging-fleet-mobile.webp', width: 1400, height: 788, alt: 'Dispositivos personales organizados y preparados para el siguiente turno', sizes: '(max-width: 860px) 100vw, 46vw' },
  closing: { src: '/images/campaign-comic/20-worker-portrait.webp', mobileSrc: '/images/campaign-comic/20-worker-portrait-mobile.webp', width: 1122, height: 1402, alt: 'Trabajador protegido frente al frío con su dispositivo personal visible', sizes: '(max-width: 860px) 100vw, 38vw' }
} as const;

function ArtworkImage({ artwork, className, priority = false }: { artwork: LandingArtwork; className?: string; priority?: boolean }) {
  return <img className={className} src={artwork.src} srcSet={`${artwork.mobileSrc} 720w, ${artwork.src} ${artwork.width}w`} sizes={artwork.sizes} alt={artwork.alt} width={artwork.width} height={artwork.height} loading={priority ? undefined : 'lazy'} fetchPriority={priority ? 'high' : undefined} decoding="async" />;
}
...
function GuideForm() {
  ...
  return <form className="lp-form" onSubmit={submit} data-lead-source="appcc_guide">
    ...
    <label>Email profesional<input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label>
    ...
    {status === 'sent' && <p className="success" role="status" aria-live="polite">Revisa tu correo para acceder a la guía.</p>}{status === 'error' && <p className="error" role="alert">No se pudo enviar la guía. Inténtalo de nuevo más tarde.</p>}
  </form>;
}

export function PublicHome() {
  return <main className="public-landing"><PublicNav /><section className="lp-hero lp-hero-art"><div className="lp-hero-copy"><p className="eyebrow">Supervisión en frío extremo</p><h1>Supervisa mejor a quienes trabajan en cámaras congeladoras.</h1><p>Controla permanencias, recibe alertas y conserva un historial operativo para que el equipo responsable disponga de más contexto.</p><div className="actions"><a className="btn" href="#guia">Recibir la guía gratuita</a><a className="btn ghost" href="/info-faqs">Descubrir cómo funciona</a></div></div><div className="lp-hero-visual"><ArtworkImage artwork={landingArtwork.hero} priority /></div></section>
    <section className="lp-section lp-intro lp-intro-art" aria-labelledby="landing-intro-title"><div><p className="eyebrow">Una operación más preparada</p><h2 id="landing-intro-title">Visibilidad cuando más importa.</h2><p>Preparar al equipo también forma parte del procedimiento. HorizonST ayuda a organizar el control de accesos y tiempos de permanencia mediante tecnologías inalámbricas, con alertas y trazabilidad pensadas para el equipo responsable.</p></div><ArtworkImage artwork={landingArtwork.intro} /></section>
    <section className="lp-section lp-alert-story" aria-labelledby="alert-story-title"><div className="lp-alert-copy"><p className="eyebrow">De la alerta a la revisión</p><h2 id="alert-story-title">Más contexto para responder y aprender.</h2><p>Cuando se genera una alerta, el equipo responsable recibe el aviso y aplica el procedimiento interno definido por la operación.</p><ul className="lp-story-points"><li><strong>Recibir el aviso</strong><span>La información llega al equipo encargado de comprobar la situación.</span></li><li><strong>Aplicar el procedimiento</strong><span>La respuesta se coordina conforme a las pautas internas del centro.</span></li><li><strong>Revisar lo ocurrido</strong><span>El historial aporta contexto para analizar la incidencia posteriormente.</span></li></ul><a className="btn" href="/info-faqs">Descubrir cómo funciona</a></div><div className="lp-alert-collage"><ArtworkImage artwork={landingArtwork.alert} className="lp-alert-main" /><ArtworkImage artwork={landingArtwork.response} /><ArtworkImage artwork={landingArtwork.review} /></div></section>
    <section className="lp-section lp-sectors" aria-labelledby="sectors-title"><div className="lp-sectors-copy"><p className="eyebrow">Distintas operaciones, una misma necesidad</p><h2 id="sectors-title">Supervisión adaptada a entornos de frío diferentes.</h2><p>HorizonST se adapta a cámaras frigoríficas, almacenes y procesos con distintas necesidades de seguimiento operativo.</p></div><div className="lp-sector-strip"><figure><ArtworkImage artwork={landingArtwork.sectors[0]} /><figcaption>Alimentación</figcaption></figure><figure><ArtworkImage artwork={landingArtwork.sectors[1]} /><figcaption>Almacenamiento especializado</figcaption></figure></div></section>
    ...
    <section id="guia" className="lp-section lp-guide-conversion" aria-labelledby="guide-conversion-title"><div className="lp-guide-visual"><ArtworkImage artwork={landingArtwork.guide} /><p className="eyebrow">Preparación práctica</p><h2 id="guide-conversion-title">Convierte la prevención en un procedimiento más claro.</h2><p>Recibe gratis una guía centrada en los aspectos que forman parte de la operativa diaria.</p><ul className="lp-guide-benefits"><li>Accesos y permanencias</li><li>Alertas y respuesta interna</li><li>Incidencias y revisión posterior</li></ul></div><GuideForm /></section>
    <section className="lp-section lp-plans-art" aria-labelledby="plans-art-title"><div><p className="eyebrow">Preparados para crecer</p><h2 id="plans-art-title">Una solución que acompaña a tu operación.</h2><p>Consulta opciones según la cobertura, el número de trabajadores y las necesidades de cada centro.</p><a className="btn" href="/planes">Ver planes</a></div><ArtworkImage artwork={landingArtwork.plans} /></section>
    <section className="lp-section lp-emotional-close" aria-labelledby="emotional-close-title"><div className="lp-emotional-visual"><ArtworkImage artwork={landingArtwork.closing} /></div><div><p className="eyebrow">Las personas, en el centro</p><h2 id="emotional-close-title">Más visibilidad para cuidar a quienes trabajan en frío.</h2><p>HorizonST aporta información al equipo responsable y complementa los procedimientos definidos por cada operación.</p><a className="btn" href="/planes">Conocer los planes</a></div></section><PublicFooter /></main>;
}
