...
const makeAdminQuoteHarness = (options: { existing?: any; orderError?: Error } = {}) => {
  const calls: any[] = []; const historyCalls: any[] = []; const orderCalls: any[] = []; let released = false;
  ...
  app.use('/api/admin', createAdminQuotesRouter({ pool, authMiddleware: (req, _res, next) => { req.user = { sub: adminId, role: 'admin', status: 'active' } as any; next(); }, roleMiddleware: (_req, _res, next) => next(), insertQuoteStatusHistory: async (input, queryClient) => { historyCalls.push({ input, sameClient: queryClient === client }); }, generateQuotePdf: async () => Buffer.from('%PDF-test'), createOrderFromAcceptedQuote: async (input) => { orderCalls.push({ input, sameClient: input.client === client, callIndex: calls.length }); if (options.orderError) throw options.orderError; return { order, items: [], created: true }; } } as any));
  app.use(errorHandler);
  return { app, calls, historyCalls, orderCalls, get released() { return released; } };
};
