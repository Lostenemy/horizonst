  ...
  return async (req, res, next) => {
    ...
    try {
      if (Date.now() >= nextCleanup) {
        ...
        await store.query('DELETE FROM store.auth_rate_limits WHERE bucket_key IN (SELECT bucket_key FROM store.auth_rate_limits WHERE expires_at <= clock_timestamp() LIMIT 500)');
      }
      for (const [key, limit] of buckets) {
        const result = await store.query(
          `INSERT INTO store.auth_rate_limits AS budget (bucket_key, attempts, expires_at)
