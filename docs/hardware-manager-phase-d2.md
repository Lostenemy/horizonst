...
Para `connect`, Hardware Manager devuelve `202 connection_unverified` y `connectionState="unverified"` si no puede atribuir la conexión, incluso ante un `3151` positivo con MAC coincidente. Horneo registra `attempted_unverified` y se detiene: no reintenta `1150`, no cambia a otra gateway, no envía LED/buzzer/vibración ni solicita `1200`, pues no se ha acreditado una sesión BLE concreta. La alerta funcional permanece registrada aunque la entrega física no pueda verificarse. Los rechazos explícitos de comandos distintos conservan su camino de error. La emergencia manual mantiene `dispatchPhysicalAlarm: false`.
