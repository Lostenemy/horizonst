...
reportsRouter.get('/inspection.pdf', async (_req: Request, res: Response, next) => {
  try {
    ...
    const criticalRows = rows.filter((row) => row.duration_minutes >= 45).length;
    const avgMinutes = rows.length ? rows.reduce((acc, row) => acc + row.duration_minutes, 0) / rows.length : 0;
    ...
    try {
      doc.image(logoPath, 36, 28, { fit: [90, 45] });
    } catch {
      // logo optional
    }
    ...
    doc.moveDown(2);
    doc.roundedRect(36, 95, 523, 70, 8).fillAndStroke('#EDF4FB', '#D9E7F5');
    doc.fillColor('#0F3D5E').fontSize(11).text(`Sesiones analizadas: ${totalRows}`, 52, 115);
    doc.text(`Sesiones >= 45 min: ${criticalRows}`, 240, 115);
    doc.text(`Promedio de minutos: ${avgMinutes.toFixed(2)}`, 400, 115, { width: 145, align: 'right' });
    ...
    rows.slice(0, 120).forEach((row, idx) => {
      ...
      doc.fillColor(row.duration_minutes >= 45 ? '#C62828' : '#111827').text(row.duration_minutes.toFixed(2), colX[5], y, { width: 34, align: 'right' });
      y += 18;
    });

    doc.fillColor('#6B7280').fontSize(8).text('HorizonST · Cold Compliance', 36, 810, { align: 'left' });
    doc.text(`Página ${doc.bufferedPageRange().count || 1}`, 36, 810, { align: 'right' });
