...
import { accumulateWorkerWorkday, madridWorkdayWindow, WorkdaySession, WORKDAY_LIMIT_SECONDS } from '../workday-duration';

const worker = (started_at: string, ended_at: string | null, worker_id = 'worker-1'): WorkdaySession => ({
  worker_id, full_name: worker_id, dni: worker_id, started_at, ended_at
});
...
test('sums entries and exits, keeps a worker after leaving, and includes an open session', () => {
  const rows = accumulateWorkerWorkday([
    ...
    worker('2026-09-25T09:15:00Z', null),
...
test('a worker appears from the first detection, even before one second has accrued', () => {
  const rows = accumulateWorkerWorkday([
    worker('2026-09-25T09:00:00Z', null)
...
test('six hours is 100 percent and extra time remains visible without capping or triggering action', () => {
  ...
  assert.equal(overLimit.progress_percent, 102);
});
