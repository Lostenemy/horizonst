...
- **Control GPIO de lectores Keonn:** cuando la API externa devuelve *acceso permitido* se enciende el GPO 4 durante 5 s; con *acceso denegado* se activan los GPO 5 (10 s) y 6 (5 s) de forma concurrente. Las peticiones se envían vía `GET {baseUrl}/devices/setGPO/{line}/{state}` (AdvanNet `setGPO`). Configura `RFID_READER_CONTROLLER_BASE_URL`, `RFID_READER_CONTROLLER_TIMEOUT` y `RFID_READER_CONTROLLER_ENABLED` para activar el control; si falta la URL base o el flag está en `false`, el módulo queda deshabilitado sin bloquear el resto del flujo.
